"""
Test cases for the Finding class in the libcovulor package.
"""
...
data_create = {
    ...
    "description": (
        "##Category: Open Redirect\n###Abstract:\nThe file nusoap.php passes unvalidated data "
        "to an HTTP redirect function on line 3729. Allowing unvalidated input to control the URL used in a redirect can aid phishing attacks.\n"
        ...
        "isset($HTTP_SERVER_VARS['HTTPS']) ? $HTTP_SERVER_VARS['HTTPS'] : 'off';\n\t\t\t} else {\n```\n##Explanation:\n Redirects allow web applications to direct users to different pages "
        "within the same application or to external sites. Applications utilize redirects to aid in site navigation and, in some cases, to track how users exit the site. "
        "Open redirect vulnerabilities occur when a web application redirects clients to any arbitrary URL that can be controlled by an attacker. \n\nAttackers can utilize open redirects "
        "to trick users into visiting a URL to a trusted site and redirecting them to a malicious site. By encoding the URL, an attacker can make it more difficult for end-users to notice "
        "the malicious destination of the redirect, even when it is passed as a URL parameter to the trusted site. Open redirects are often abused as part of phishing scams to harvest sensitive "
        "end-user data.\n\n\n\n\nExample 1: The following PHP code instructs the user's browser to open a URL parsed from the dest request parameter when a user clicks the link. \n\n\n    <%\n        ...\n        $strDest = $_GET[\"dest\"];\n        header(\"Location: \" . $strDest);\n        ...\n    %>\n\n\nIf a victim received an email instructing the user to follow a link to "
        "\"http://trusted.example.com/ecommerce/redirect.php?dest=www.wilyhacker.com\", the user would likely click on the link believing they would be transferred to the trusted site. However, when the user clicks the link, the code above will redirect the browser to \"http://www.wilyhacker.com\". \n\nMany users have been educated to always inspect URLs they receive in emails to make sure the link specifies a trusted site they know. However, if the attacker Hex encoded the destination url as follows:\n \"http://trusted.example.com/ecommerce/redirect.php?dest=%77%69%6C%79%68%61%63%6B%65%72%2E%63%6F%6D\"\nthen even a savvy end-user may be fooled into following the link."
    ),
...
finding_id = finding.create_finding(data_create)
print(finding_id)

print("---------------------- Finding List")
print(finding.get_findings_by_client_id(
    '65f079f3ef898e6a6bb37e5b', 
    {"pagination": {"page_size": 1, "page": 1, "paginate": False}, 
     "filters": {"tool": "fortify"}, 
     "sort": {"order": 1, "field": "date"}}
))

print("---------------------- Finding update")
print(finding.update_finding_by_id_and_client_id(
    {'is_false_positive': True}, finding_id, '65f079f3ef898e6a6bb37e5b'
))

print("---------------------- Finding by id and client id")
print(finding.get_finding_by_id_and_client_id(
    finding_id, '65f079f3ef898e6a6bb37e5b'
))
